...
**메시지 파이프라인**: 텍스트 메시지를 즉시 저장/응답한 뒤, 발신자에게 ElevenLabs voice clone이 있으면 비동기로 (1) Gemini 태깅+번역 → (2) TTS → (3) Storage 업로드 처리. (1) **감정 마커 → audio tag 치환은 Gemini가 번역과 같은 호출에서 수행** (regex `prepareTextForTTS` 폐지). raw 텍스트를 그대로 Vertex AI Gemini 2.5 Flash로 보내고, 시스템 프롬프트가 STEP 1(텍스트에 **실제로 나타난** 채팅체 감정 마커 — `ㅋ`/`ㅎ`(융합 자모 `욬`→`요` 복원 포함)/`www`/`草`/`hahaha`/`lol`/`5`+/`हाहा`/`ㅠ`/`ㅜ`/`xD`/`:(`/`T_T` 등 ko/ja/en/th/hi 웃음·슬픔 마커만 → `[laughs]`/`[sad]`, 문맥/의미 추론 삽입 금지) + STEP 2(register-preserving 번역 — ko 해요체 default + 반말 허용, ja です/ます default + 캐주얼 허용, en 회화체, zh 您 default + 你 허용, 삽입 태그 verbatim 보존)를 한 번에 처리. 한숨은 모든 언어가 의성어 단어(에휴/はぁ/ugh)로 표현되어 태그 우회 불필요 → dictionary 제외. Gemini 응답은 `sanitizeAudioTags`(`src/utils/textNormalization.ts`)로 **화이트리스트 검증** — `[laughs]`/`[sad]`(+eleven_v3 표준 태그) 외 태그·malformed 태그를 strip해 규율 이탈 시 TTS/UI 오염 차단. **송수신자 언어 일치 여부와 무관하게** 항상 Gemini 경유(대화 중 언어 전환·code-switching 대비). `BLOCK_ONLY_HIGH` safety, `temperature=0.4`. (2) TTS 직전 `stripNonAudibleTags`로 **`[laughs]`만 남기고 `[sad]` 등 나머지 audio tag는 제거** — 사용자 정책상 `[laughs]`만 audible(웃음 소리), `[sad]`는 **display-only**(클론 보이스가 흐느낌은 안 냄). `[sad]` 마커 감지·정규화는 STEP 1이 여전히 수행(raw `ㅠㅠ`가 TTS로 새서 자모 괴음 나는 것 방지 목적)하되 소리로는 합성 안 함. 순수 sad 메시지(`ㅠㅠ`)는 strip 후 빈 텍스트 → `hasSpeakableContent` false → TTS 스킵(`audio_url=null`), display 슬랭은 유지. strip 뒤 텍스트로 ElevenLabs `eleven_v3` TTS(`stability=1.0`, 사용자 선택 emotion은 별도 `[emotion]` 프리픽스)로 발신자 클론 보이스 합성. (3) `translated_text` 저장 시 `replaceTagsForDisplay`로 audio tag를 **타깃 언어 슬랭으로 치환**(TTS strip과 무관하게 `[sad]`→슬랭 유지) — UI에 raw 태그 노출은 막되 원본의 감정 표현 의도를 번역본에서도 보존 (예: ko→ja에서 `[laughs]` → `www`, `[sad]` → `(泣)`). 매핑: laughs={ko:ㅋㅋㅋ, ja:www, en:lol, th:555, hi:हाहा}, sad={ko:ㅠㅠ, ja:(泣), en::(, th:T_T, hi::(}. identity 판정(`senderLang===recipientLang` **이고** `replaceTagsForDisplay(translation)===원문`)이면 `translated_text = NULL`로 저장해 FE 번역 인디케이터 숨김 — code-switching(프로필=ko인데 영어로 타이핑)이면 display가 원문과 달라 번역 노출. `audio_status` 필드로 진행 상태 추적. 차단된 유저 간 메시지 전송은 403 차단.
...
**추천 알고리즘**: 디스커버에서 스와이프/차단/선호도/reciprocity 풀(나를 like 한 사람들)을 병렬 조회 후, 후보를 limit×5개(최대 200) 가져옴. 사전 SQL 필터는 (a) 성별/연령 (b) **viewer 의 `profiles.language` 와 동일한 후보를 하드 제외** — 크로스언어 매칭 차별점을 알고리즘 차원에서 강제. viewer 언어가 비어있으면 이 필터는 적용 안 함. 국가 선호는 SQL 단계에서 거르지 않고 티어 정렬 신호로만 사용. 서버에서 2-단계 티어 + 동일 티어 내 2차 점수 계산 → `(tier ASC, intra DESC)` 정렬 → limit개 반환. `src/routes/swipe.ts`의 `computeTier()` + `computeIntraScore()` + `matchesNationality()` + `hashJitter()`. 티어는 국적 부합 여부만으로 가른다: (1) 선호 국가 부합 (2) 선호 국가 미부합. 국가 부합은 후보 `profiles.nationality` 가 `preferred_nationalities` 안에 있으면 true, 빈 선호는 항상 부합(=제약 없음)이라 모두 T1. **선호 언어(`preferred_languages`) 티어 차원은 제거됨** — 언어가 국적에서 파생되면서 선호 국적이 동일 신호를 커버해 중복이 됐고(예: KR 국적 후보는 항상 ko), FE 선호 언어 UI 도 삭제됨. (viewer 본인 언어와 같은 후보 하드 제외는 위 사전 SQL 필터의 별도 메커니즘으로 그대로 유지.) 동일 티어 내 2차 점수는 (기본 신호) 관심사 겹침(최대 +30), 사진 3장+ (+10), 신규 가입 7일 이내 (+10), 결정적 jitter (0~15) — 기본 신호 합산 상한 65 — + (reciprocity 신호) **나를 이미 like 한 후보 +50**. 합산 최대 115. 티어 경계 보호는 합산 상한이 아니라 정렬 키 우선순위 (`tier` 1차 > `intra` 2차) 가 담당 — reciprocity 가 +50 가산되어도 다른 티어 후보 위로 못 올라간다. **reciprocity boost** 는 "내가 만나고 싶은 사람 중 나를 좋아하는 사람" 을 같은 티어 안에서 1순위로 노출시켜, 사용자 명시 선호를 유지하면서 양방향 마주칠 확률을 끌어올리는 메커니즘 (1000명대 규모에서 mutual 노출 누수가 가장 큰 회수). jitter는 같은 viewer-candidate 쌍에 대해 결정적이므로 페이지네이션 시 순서 안정.
...
- `/api/discover` — 추천 후보 조회, 스와이프 (동시 like 중복 매치 방지), `/quota` 일일 한도 조회, `/likes-received` 받은 좋아요 목록 (나를 like 한 사용자 중 미스와이프·비차단 후보, 디스커버 카드와 동일 shape)
- `/api/matches` — 매치 목록(N+1 해결 RPC, 커서 페이지네이션), 언매치, 메시지 CRUD, 읽음 처리
- `/api/block` — 유저 차단/해제/목록 (차단 시 매치 자동 soft delete, 해제 시 404 처리)
- `/api/report` — 유저 신고
- `/api/preferences` — 매칭 선호도 (나이/성별/국가). 사전 SQL 필터: 나이·성별 + viewer 본인 언어와 동일한 후보 하드 제외. 국가 선호는 티어 정렬 신호. (선호 언어 기능은 mig 042 에서 완전 제거 — 컬럼 drop + 라우트/스키마에서 삭제. 언어가 국적에서 파생되어 선호 국적이 동일 신호를 커버.)
- `/docs` — Swagger UI
...
- 번역은 `src/services/translation.ts`의 `translateMessage()` (Vertex AI Gemini 2.5 Flash, `responseMimeType: application/json`, `temperature=0.4`, `BLOCK_ONLY_HIGH` safety). source/target 은 송수신자의 `profiles.language` 스칼라 값. TTS는 `src/services/elevenlabs.ts`의 `synthesizeSpeech()` — `eleven_v3` 모델, `stability=1.0`, 언어 코드는 안 보내고 텍스트에서 자동 감지. emotion 값이 있으면 텍스트 앞에 `[emotion]` 프리픽스로 전달.
