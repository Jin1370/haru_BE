...
//   2) haru_BE/supabase/migrations/ 중 트리거를 마지막으로 정의한 파일 — SQL 리터럴
...
describe('photo unlock threshold 3-way sync (BE constants / trigger SQL / FE constants)', () => {
  ...
  // 트리거 함수는 마이그레이션이 forward-only 로 재정의(028 이 014 를 교체)하므로
  // "가장 마지막에 함수를 정의한 마이그" 가 현행 진실이다. 파일명을 박으면 다음
  // 재정의 때 이 테스트가 옛 파일을 읽어 조용히 틀린 값을 비교한다.
  const migrationsDir = resolve(REPO_ROOT, 'haru_BE', 'supabase', 'migrations');
  const sqlPath = resolve(
    ...
    readdirSync(migrationsDir)
      ...
      .reverse()
      .find((f) =>
        readText(resolve(migrationsDir, f)).includes(
          'FUNCTION match_roundtrip_on_insert',
        ),
      )!,
  );
  ...
  // `--` 주석 제거 후 파싱 — 028 헤더가 옛 임계치(`new_count >= 5`)를 설명문으로
  // 인용하고 있어, 주석을 남기면 실제 분기 대신 그 숫자를 읽는다.
  const sqlText = readText(sqlPath).replace(/--[^\n]*/g, '');
  ...
  // 현행 트리거 안의 두 `new_count >= N` 리터럴 (main, all 순서로 등장).
  ...
  it('현행 트리거 SQL 이 두 개의 `new_count >= N` 리터럴을 보유한다', () => {
    expect(sqlMain, 'first new_count >= N missing in trigger migration').not.toBeNull();
    expect(sqlAll, 'second new_count >= N missing in trigger migration').not.toBeNull();
  });
